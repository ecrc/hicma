...
int AL4SAN_RHS_zgene_Tile( int ntrian, int nip, int local_nt, int NB, MORSE_desc_t *A)
{
  ...
  //printf("\nHERERE1ntrian:%d, nip:%d, local_nt:%d, NB:%d\n", ntrian, nip, local_nt, NB);
    HICMA_RHS_zgene_Tile_Async( ntrian,  nip, local_nt,  NB, A, sequence, &request);
 ...
 *
 ******************************************************************************/
int HICMA_RHS_zgene_Tile_Async( int ntrian, int nip, int local_nt, int NB, MORSE_desc_t *A, MORSE_sequence_t *sequence, MORSE_request_t *request)
{
    MORSE_context_t *morse;
...
*/
   // printf("\nHERERE2ntrian:%d, nip:%d, local_nt:%d, NB:%d\n", ntrian, nip, local_nt, NB);
    compute_RHS_gene( ntrian, nip, local_nt, NB, A,
                    sequence, &request );
