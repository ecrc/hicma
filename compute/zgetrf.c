        ...
        )
{
    ...
    if (hicma == NULL) {
        hicma_fatal_error("HiCMA_zpotrf_Tile", "HiCMA not initialized");
        return HICMA_ERR_NOT_INITIALIZED;
    }
    hicma_sequence_create(hicma, &sequence);
    HICMA_zgetrf_Tile_Async(uplo,
            AUV, AD, Ark,
            rk, maxrk, acc,
            sequence, &request
            );
        ...
        )
{
    ...
    hicma = hicma_context_self();
    if (hicma == NULL) {
        hicma_fatal_error("HiCMA_zpotrf_Tile_Async", "HiCMA not initialized");
        return HICMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        hicma_fatal_error("HiCMA_zpotrf_Tile_Async", "NULL sequence");
        return HICMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        hicma_fatal_error("HiCMA_zpotrf_Tile_Async", "NULL request");
        return HICMA_ERR_UNALLOCATED;
    }
    ...
    if (AD->nb != AD->mb) {
        hicma_error("HiCMA_zpotrf_Tile_Async", "only square tiles supported");
        return hicma_request_fail(sequence, request, HICMA_ERR_ILLEGAL_VALUE);
    }
    if (uplo != HicmaUpper && uplo != HicmaLower) {
        hicma_error("HiCMA_zpotrf_Tile_Async", "illegal value of uplo");
...
*/
    ...
    hicma_pzgetrf(uplo, AUV, AD, Ark, sequence, request,
            rk, maxrk, acc
            );
