...
RunTest(int *iparam, double *dparam, morse_time_t *t_, char* rankfile)
{
    ...
    PROGRESS("Acoustic_Init finished");
    //printf ( "Hala Values are: %g%+gi %g%+gi \n", creal(zzsparse_tempt[0]), cimag(zzsparse_tempt[0]),creal(zzsparse_tempt[1]), cimag(zzsparse_tempt[1]));
    int flage=0;
    if(check==1 || solve ==1) flage=1;
    PASTE_CODE_ALLOCATE_MATRIX_TILE( descDense, flage, MORSE_Complex64_t, MorseComplexDouble, (nip* ntrian), (nip* ntrian), (nip* ntrian) ); 
    ...
    MORSE_dlaset_Tile(MorseUpperLower, 0, 0, descArk);
...
//printf("Setting upper part of AhicmaL to zero\n"); fflush(stdout);
            ...
            PASTE_TILE_TO_LAPACK( descAD,  AhicmaT /*low rank U*/, check, MORSE_Complex64_t, LDA, M );
