...
Test(int64_t n, int *iparam, 
    ...
    static void
        show_help(char *prog_name) {
            ...
            printf( "Options are:\n"
                    ...
                    " --ntrian          @noha please add description\n"
                    " --nipp          @noha please add description\n"
                    " --ntrian          @noha please add description\n"
                    " --ntrian          @noha please add description\n"
                    "--precent1 percentage of initial maxrank\n"
                    "--precent2 percentage of calaculated max rank\n"
                    "--isPrecent ask user if they want to allocate according to max rank precentage"
                    "\n"
                    ...
                    "                   --nipp            Quad points\n"
	            "                   --ntrian          Traingles\n"
		   "--solve   Enabling LU solver\n"
                    "nrows==ncols = nipp X ntriangles , So total matrix size is (nipp X ntriangles) X (nipp X ntriangles) \n"
                    "NB == MB has to be multiple of nipp \n"
                    "For examples if we have ntriangles=6 and nipp= 3, so matrix size is 18 X18, smaller tile size you could go is 3X3. \n"
                    "more_vertYou could go to bigger tile size of multiple 3, such as 6X6\n"
                    /*             "\n" */
    ...
    static void
        print_header(char *prog_name, int * iparam,
		    ...
		    "# nipp:			    %d\n"
                    ...
                    iparam[IPARAM_HICMA_NIPP],
                    iparam[IPARAM_HICMA_NTRIAN]
                    );
    ...
    int
        main(int argc, char *argv[]) {
            ...
            iparam[IPARAM_HICMA_NTRIAN  ] = 6;
            iparam[IPARAM_HICMA_NIPP  ] = 3;
            iparam[IPARAM_HICMA_PRECENT1] = 75;
            iparam[IPARAM_HICMA_PRECENT2] = 20;
            iparam[IPARAM_HICMA_ISPRECENT] = 1;
            ...
            for (i = 1; i < argc && argv[i]; ++i) {
                ...
                }else if (startswith( argv[i], "--precent1=" )) {
                    sscanf( strchr( argv[i], '=' ) + 1, "%d", &(iparam[IPARAM_HICMA_PRECENT1]) );
                }else if (startswith( argv[i], "--precent2=" )) {
                    sscanf( strchr( argv[i], '=' ) + 1, "%d", &(iparam[IPARAM_HICMA_PRECENT2]) );
                }else if (startswith( argv[i], "--isPrecent=" )) {
                    sscanf( strchr( argv[i], '=' ) + 1, "%d", &(iparam[IPARAM_HICMA_ISPRECENT]) );
                }else if (startswith( argv[i], "--solve=" )) {
                    sscanf( strchr( argv[i], '=' ) + 1, "%d", &(iparam[IPARAM_HICMA_SOLVE]) );
