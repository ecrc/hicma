...
RunTest(int *iparam, double *dparam, hicma_time_t *t_, char *rankfile) {
    ...
    //END: ss
    if (iparam[IPARAM_HICMA_STARSH_PROB] == PROBLEM_TYPE_ST_2D_EXP) {
        hicma_problem.beta = 0.1;
        // Smoothing parameter for Matern kernel
        hicma_problem.nu = 0.5;
        // Shift added to diagonal elements
    ...
    if (iparam[IPARAM_HICMA_STARSH_PROB] == PROBLEM_TYPE_3D_RBF_VIRUS) {
        ...
        hicma_problem.mesh_file = mesh_file;   // path to mesh file

    }
    if (iparam[IPARAM_HICMA_STARSH_PROB] == PROBLEM_TYPE_3D_RBF_CUBE) {
        hicma_problem.kernel_type = iparam[IPARAM_RBFKERNEL]; // RBF kernel_type
        hicma_problem.reg = 1 + fixedacc * 10;  //RBF regularization value
        hicma_problem.isreg = 1;
        hicma_problem.rad = rad;  // RBF scaling factor
    ...
    if (check) {
        swork = calloc(2 * M, sizeof(double));
        {
            ...
            /*HICMA_dnormest(M, M, orgAdense, &normOrgAdense, swork);*/
    ...
    if (iparam[IPARAM_HICMA_STARSH_PROB] == PROBLEM_TYPE_3D_RBF_VIRUS) {
        if (solve) {
            ...
            PASTE_TILE_TO_LAPACK(descB, descB_array_cpy, 1, double, M, 3);
            if (check_solve) {
...
//                HICMA_dgemm_Tile(HicmaNoTrans, HicmaNoTrans, alpha, descmat, descB, beta, descBcpy);
                cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, M, M, 1, alpha,descmat_array,
                            M, descB_array, M, beta, descB_array_cpy, M);

//                dparam[IPARAM_IRNORM] = HICMA_dlange_Tile(HicmaInfNorm, descBcpy);
                dparam[IPARAM_IRNORM] = LAPACKE_dlange(LAPACK_COL_MAJOR,'i', M,M, descB_array_cpy, M);

                dparam[IPARAM_IRES] =
                        dparam[IPARAM_IRNORM] / (dparam[IPARAM_IANORM] * dparam[IPARAM_IXNORM] + dparam[IPARAM_IBNORM]);
