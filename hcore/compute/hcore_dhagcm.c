...
int steal_lrtile = 0; //FIXME for experimental reasons. Otherwise set to 0!!!!!


extern void _printmat(double * A, int m, int n, int ld);

void HCORE_dhagcm( int m, int n, /*dimension of squareAD*/
        double *AU,
        double *AV,
        double *Ark,
        int ldu,
        ...
        int A_mt)
{
    ...
    free(work);
    free(iwork);
    if(rank == -1 || HICMA_get_print_index_end() == 1 || 
