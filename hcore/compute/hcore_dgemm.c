...
int dgemm_print_index = 0;
int dgemm_print_mat = 0;
int dhc_nelm_limit = 512;


#undef CBLAS_SADDR
#define CBLAS_SADDR(_val) (_val)
void dhc_printmat(double * A, int m, int n, int ld){
    printf("M:%d N:%d LD:%d\n[", m, n, ld);
    int i, j, nelm = 0;
    for(i=0;i<m;i++){
        printf("[");
...
void __dsvd(
    ...
    if(dgemm_print_index){
        ...
        dhc_printmat(_CU,  _M, _Crk, ld_CU);
        dhc_printmat(_rA,  rA_nrows, rA_ncols, ld_rA);
    }
    /// rA     END
    /// rB    START
    int CV_nrows = _M;         //ASSUMPTION
    ...
    if(dgemm_print_mat){
        ...
        dhc_printmat(_CV,  _M, _Crk, ld_CV);
        dhc_printmat(_rB,  rB_nrows, rB_ncols, ld_rB);    }
    /// rB    END
    /// SVD   START
  ...
  cblas_dtrmm(CblasColMajor, CblasRight, CblasUpper, CblasTrans, CblasNonUnit, rA_nrows, rA_ncols,  alpha, _rB, ld_rB, _rA, ld_rA); //FIXME Correctness of rA_ncols is not checked
    ...
    if(dgemm_print_mat){
        dhc_printmat(_T,  T_nrows, T_ncols, ld_T);
    }
    ...
    if(dgemm_print_mat){
        dhc_printmat(_U,  U_nrows, U_ncols, ld_U);
        dhc_printmat(sigma,  1, size_sigma, 1);
        dhc_printmat(_V,  V_nrows, V_ncols, ld_V);
        printf("%d %e\n", rank, acc);
    }
    int finalrank = -1;
...
#ifdef KA_COUNT
    int myid = RUNTIME_thread_rank(NULL);
    counters[myid].update += ka_counts('o', CU_nrows, U_ncols, ncols_qA, 1);
#endif
    U_nrows = CU_nrows;
...
#ifdef KA_COUNT
    int myid = RUNTIME_thread_rank(NULL);
    counters[myid].update += ka_counts('o', V_nrows, CV_nrows, ncols_qB ,2);
#endif
    V_ncols  = CV_nrows;
    CV_ncols = V_nrows;
    if(dgemm_print_index){
                 ...
                 )
{
    ...
    __dqra(_M, maxrk, _CU, ld_CU, _Crk, &CU_ncols, _AU, ld_AU, _Ark, alpha, beta, qrtauA);
    ...
    __dqrb(_M, maxrk, _CV, ld_CV, _Crk, &CV_ncols,
    ...
    size_t svd_rA_nelm;
    if(duse_trmm == 1){ // allocate more because rA will be output of trmm(rA*rB^T)
