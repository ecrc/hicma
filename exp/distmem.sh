...
#SBATCH --output=/project/k1205/omairyrm/hicma-noha/hicma-runs/out1/%j
#SBATCH --error=/project/k1205/omairyrm/hicma-noha/hicma-runs/err1/%j
...
hn=$HOSTNAME
echo $HOSTNAME
HICMADIR=$HOME/hicma-dev
if [ -f "kadir.setting" ]; then
    . kadir.setting
fi
...
if [[ "$hn" = xci* ]]; then #isambard
    ...
    sruncmd="mpirun -n $numnodes numactl --interleave=all  "
    sruncmd="numactl --interleave=all  "
else
    if [ "$USER" == akbudak ];then
        HICMADIR=/project/k1205/akbudak/hicma-dev
    else
        HICMADIR=/project/k1205/omairyrm/hicma-noha/hicma-dev
    fi
...
if [ "$op" == "potrf" ]; then
    BINNAME=time_z${op}_tile #FIXME
elif [ "$op" == "getrf" ]; then
    #BINNAME=time_zgetrf_tile
    BINNAME=time_zgetrf_tile_buffer
    CHAMBINNAME=time_dgetrf_nopiv_tile
    #BINNAME=time_z${op}_tile #FIXME
elif [ "$op" == "posv" ]; then
...
if [ "$trace" != "-" ]; then
    ...
    if [ "$dry" != "dry" ]; then
        ...
        if [[ "$hn" = xci* ]]; then #isambard
            ...
            . $HOME/hicma-dev/scripts/modules-isambard-cray.sh
        else
            module load cdt
    ...
    for iexp in $exps;do 
        ...
        for _acc in ${acc[iexp]}; do
            ...
            for pdim in $pdims; do
                if [ "$prog" == "hic" ]; then
                    rankfile=$BINDIR/../../exp/ranks/$prog-$sched-$_m-$_mb-$_nb-$numnodes-$nummpi-$numthreads-$SLURM_JOBID
                    if [ "$op" == "potrf" ]; then
                        cmd="$BIN \
                            --m=$_m \
                            --n_range=$_n:$_n \
                    ...
                    if [ "$trace" != "-" ]; then
                        ...
                        cmd="$CHAMDIR/timing/$CHAMBINNAME --P=$pdim --m=$_m --n_range=$_m:$_m:$_m $cham_block_size_arg --threads=$numthreads --nowarmup "
                    fi
                fi
                minmaxsubinfo=
                if [ "$minmaxsub" != "-" ]; then
