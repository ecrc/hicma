    ...
    for isub in 0;do
        ...
        while [ $ct -le $endt ]; do
            ...
            for nt in $numthreads; do
                ...
                cmd="$PWD/exp/distmemamd.sh $nodes $nodes $nt $trace $prog - \"$ids\" $dry $PWD/$cases $timelimit  $sched $maxsub $minsub $op" 
                if [ "$dry" == "dry" ]; then
                    eval $cmd 
                    continue
                fi
                if [[ "$hn" = xci* ]]; then
                    ...
                    $cmdbatch $PWD/exp/distmemamd.sh $nodes $nodes $nt $trace $prog - "$ids" $dry $cases $timelimit  $sched $maxsub $minsub $op
                fi
            done
        done
    done
