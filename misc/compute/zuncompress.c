                      ...
                      MORSE_desc_t *AUV, MORSE_desc_t *AD, MORSE_desc_t *Ark)
{
    ...
    MORSE_Complex64_t zzero = (MORSE_Complex64_t) 0.0;
    ...
    for (i = 0; i < AD->mt; i++) {
        ...
        for (j = 0; j < AD->mt; j++) {
            if(uplo == MorseLower && i<=j)
                continue;
            else if(uplo == MorseUpper && i>=j)                 continue;
...
)
{
    ...
    for (i = 0; i < AD->mt; i++) {
        ...
        int tempi = i == AD->mt-1 ? AD->m-i*AD->mb : AD->mb;
        for (j = 0; j < AD->nt; j++) {
            if(uplo == MorseLower && i<=j){printf("%s %d", __FILE__, __LINE__); continue;}
            else if(uplo == MorseUpper && i>=j){printf("%s %d", __FILE__, __LINE__); continue;}
