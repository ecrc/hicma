...
void __generate(
    ...
    for(idesc=0;idesc<num_descs;idesc++){ //foreachMatrix
        ...
        else if(probtype == HICMA_STARSH_PROB_ST_3D_EXP){
            fprintf(stderr, "%s %d Spatial Statistics Application with 3D Exp Kernel: beta:%g nu:%g noise:%g\n", __FILE__, __LINE__, 
                    beta,
                    nu, 
                    noise);
            // Correlation length
        ...
        else if(probtype == HICMA_STARSH_PROB_ST_3D_SQEXP){
            fprintf(stderr, "%s %d Spatial Statistics Application with 3D Sqexp Kernel: beta:%g nu:%g noise:%g\n", __FILE__, __LINE__, 
                    beta,
                    nu, 
                    noise);
            // Correlation length
...
//int starsh_ssdata_init(STARSH_ssdata **data, STARSH_int count, int ndim,
        ...
        //    starsh_problem_info(problem);
            ...
            *mpiF = F;
        }   else if(probtype == HICMA_STARSH_PROB_3D_RBF){
            ...
            if(0)fprintf(stderr, "%s %d RBF mesh defomation Application with Kernel:%d reg:%f rad:%f mesh_points:%d mesh_file:%s\n", __FILE__, __LINE__, 
                    kernel_type, 
                    reg, 
                    rad, 
                    mesh_points,
            ...
            starsh_generate_3d_rbf_mesh_coordinates((STARSH_mddata **)&data, mesh_file, mesh_points, problem_ndim, kernel_type, numobj, isreg, reg, rad, ordering);
