...
void HICMA_TASK_zgytlr_diag_buffer( const MORSE_option_t *options,
                        int m, int n,
                        ...
                        int compress_diag,
                        int ntrian, int nip, int local_nt, int NB
//                        const MORSE_desc_t *Dense
                        )
{
    struct starpu_codelet *codelet = &cl_zgytlrdiagb;
    ...
    starpu_insert_task(
            ...
            STARPU_W,         RTBLKADDR(AD, MORSE_Complex64_t, ADm, ADn),
            ...
            STARPU_NAME, "hcore_zgytlrb_diag",
#endif
            0);
}
...
static void cl_zgytlrb_cpu_func(void *descr[], void *cl_arg)
{
   ...
   int ntrian, nip, local_nt, NB;
    ...
    AD = (MORSE_Complex64_t *)STARPU_MATRIX_GET_PTR(descr[1]);
    Ark = (double *)STARPU_MATRIX_GET_PTR(descr[2]);
    //Dense = (MORSE_Complex64_t *)STARPU_MATRIX_GET_PTR(descr[3]);
    MORSE_Complex64_t *work = (MORSE_Complex64_t *)STARPU_MATRIX_GET_PTR(descr[3]);
 ...
 * Codelet definition
 */
CODELETS_CPU(zgytlrdiagb, 4, cl_zgytlrb_cpu_func)
//#endif
