...
void HICMA_TASK_hcore_ztrsm(const HICMA_option_t *options,
...
static void cl_ztrsm_hcore_cpu_func(void *descr[], void *cl_arg)
{
    ...
    // update HICMA_Complex64_t *B = &(BUV[0]) to access U  for the uuper paert
   int nBU;
   ...
   /*if(isPrecentageused){
    ...
    HICMA_Complex64_t *B = &(BUV[nelm_BU]);
