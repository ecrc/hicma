                      ...
                      const MORSE_desc_t *BUV, int Bm, int Bn, int ldb, const MORSE_desc_t *Brk)
{
    ...
    starpu_insert_task(
                       ...
                       STARPU_RW,        RTBLKADDR(BUV, MORSE_Complex64_t, Bm, Bn),
...
static void cl_ztrsm_hcore_cpu_func(void *descr[], void *cl_arg)
{
    ...
    BUV = (MORSE_Complex64_t *)STARPU_MATRIX_GET_PTR(descr[1]);
#if !defined(HICMA_ALWAYS_FIX_RANK)
    ...
    // update MORSE_Complex64_t *B = &(BUV[0]) to access U  for the uuper paert
   int nBU;
   if(0)printf("%s %s %d: _Brk:%d\n", __FILE__, __func__, __LINE__, _Brk);
   if(isPrecentageused){
    int sp=precent2*_Brk;
    ...
    MORSE_Complex64_t *B = &(BUV[nelm_BU]);
   ...
   if(HICMA_get_print_index() == 1 || HICMA_get_print_index_end() == 1 || trsm_print_index_end){
    ...
    // if(HICMA_get_print_mat() == 1){
    //     if (Am==1&&An==0){
         //printf("%d\ttrsm-outputttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt\n", __LINE__);
        //_printmat_complex(B, m, _Brk, ldb);
      //   }
    //}
