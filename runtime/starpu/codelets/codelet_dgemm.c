...
/*CHAMELEON_CL_CB(dgemm_hcore,         starpu_matrix_get_nx(task->handles[2]), starpu_matrix_get_ny(task->handles[2]), starpu_matrix_get_ny(task->handles[0]),     2. *M*N*K) [> If A^t, computation is wrong <]*/
...
void HICMA_TASK_dgemm(const MORSE_option_t *options,
...
static void cl_dgemm_hcore_cpu_func(void *descr[], void *cl_arg)
{
    ...
    if(print_mat){
        printf("%d\tdgemm-input\n");
        int _Ark = (int) *Ark;
        printf("AU:\n");_printmat(AU, n, _Ark, lda);
        printf("AV:\n");_printmat(AV, n, _Ark, lda);
        int _Brk = (int) *Brk;
        printf("BU:\n");_printmat(BU, n, _Brk, ldb);
        printf("BV:\n");_printmat(BV, n, _Brk, ldb);
    }
    
    int isTransA = transA == MorseTrans;
    int isTransB = transB == MorseTrans;
    
    /*if(HICMA_get_use_fast_hcore_dgemm() == 1){
        printf("\n HHHHHHHHHHHHHHHHHHHHIIIIIIIIIIIIIIIIIIIII\n");
...
// CODELETS(dgemm_hcore, 4, cl_dgemm_hcore_cpu_func, cl_dgemm_hcore_cuda_func, STARPU_CUDA_ASYNC)
#else
CODELETS_CPU(dgemm_hcore, 7, cl_dgemm_hcore_cpu_func)
// CODELETS(dgemm_hcore, 7, cl_dgemm_hcore_cpu_func, cl_dgemm_hcore_cuda_func, STARPU_CUDA_ASYNC)
#endif
//#endif
