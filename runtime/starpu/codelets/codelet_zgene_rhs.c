...
ZCODELETS_HEADER(gene_rhs)

void AL4SAN_RHS_gene(int *nip, int *ntrian, MORSE_Complex64_t *zz, int *q, int *p, int *local_nt, int *nb);

void INSERT_TASK_RHS_gene( const MORSE_option_t *options,
                        int ntrian, int nip, int local_nt, int NB, const MORSE_desc_t *A, int Am, int An, int counterp, int counterq)
{

    struct starpu_codelet *codelet = &cl_zgene_rhs;
...
static void cl_RHS_zgene_cpu_func(void *descr[], void *cl_arg)
{
    int ntrian;
    int nip;
    MORSE_Complex64_t *A;
    ...
    starpu_codelet_unpack_args(cl_arg, &ntrian, &nip, &q, &p, &local_nt, &NB, &counterp, &counterq);
//    p=p+1;
//    q=q+1;
//    p=p+counterp;
//    q=q+counterq;
    ...
    AL4SAN_RHS_gene(&nip, &ntrian, A, &q, &p, &local_nt, &NB);
       if(0){for(int k=0;k<NB;k++)
          {
            printf("[%+.4f%+.4f],", creal(A[k]), cimag(A[k]));
          }
          printf("\n");}
            //printf("%d\tgenetation-output\n", __LINE__);
     //_printmat_complex(A, NB, NB, 1);
}
 ...
 * Codelet definition
 */
CODELETS_CPU(zgene_rhs, 1, cl_RHS_zgene_cpu_func)
