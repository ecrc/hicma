...
DCODELETS_HEADER(trsmu_hcore)

//UPDATE this definition. I only copy-paste from runtime/starpu/codelets/codelet_zcallback.c
/*CHAMELEON_CL_CB(dtrsm_hcore,         starpu_matrix_get_nx(task->handles[1]), starpu_matrix_get_ny(task->handles[1]), 0,                                               M*M*N)*/
...
void HICMA_TASK_dtrsmu(const MORSE_option_t *options,
...
{
    int nBUV = BUV->nb;
    struct starpu_codelet *codelet = &cl_dtrsmu_hcore;
    /*void (*callback)(void*) = options->profiling ? cl_dtrsm_hcore_callback : NULL;*/
...
static void cl_dtrsmu_hcore_cpu_func(void *descr[], void *cl_arg)
{
    ...
    // update double *B = &(BUV[0]) to access U  for the uuper paert
    int nBU = nBUV/2;
    ...
    if(HICMA_get_print_index() == 1){
        printf("%d+TRSMu\t|AD(%d,%d) BV(%d,%d)%d m:%d lda(11):%d ldb(12):%d\n",MORSE_My_Mpi_Rank(),Am,An, Bm, Bn, _Brk, m, lda, ldb);
    }
 ...
 */
#if defined(HICMA_ALWAYS_FIX_RANK)
CODELETS_CPU(dtrsmu_hcore, 2, cl_dtrsmu_hcore_cpu_func)
// CODELETS(dtrsm_hcore, 2, cl_dtrsm_hcore_cpu_func, cl_dtrsm_hcore_cuda_func, STARPU_CUDA_ASYNC)
#else
CODELETS_CPU(dtrsmu_hcore, 3, cl_dtrsmu_hcore_cpu_func)
// CODELETS(dtrsm_hcore, 3, cl_dtrsm_hcore_cpu_func, cl_dtrsm_hcore_cuda_func, STARPU_CUDA_ASYNC)
#endif
//#endif
