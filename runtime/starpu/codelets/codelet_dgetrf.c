...
DCODELETS_HEADER(getrf_hcore)
//#if defined(HICMA_COMPLEX)
void HICMA_TASK_dgetrf(const MORSE_option_t *options, int n, int nb,
                       const MORSE_desc_t *A, int Am, int An, int lda,
                       int iinfo)
{
    (void)nb;
    struct starpu_codelet *codelet = &cl_dgetrf_hcore;
    ...
    starpu_insert_task(
                       ...
                       STARPU_NAME, "hcore_dgetrf",
#endif
                       0);
}

#if !defined(CHAMELEON_SIMULATION)
static void cl_dgetrf_hcore_cpu_func(void *descr[], void *cl_arg)
{
    ...
    /* info = LAPACKE_dpotrf_work(
     ...
     n, A, lda); */
    int *ipiv=(int*)malloc(chameleon_max(1, chameleon_min(n, n))*sizeof(int));
    ...
    //  }
    if(info != 0){
        fprintf(stderr, "%s\t|%d\t|Error in LAPACK getrf. info:%d\n", __FILE__, __LINE__, info);
        if(0) {
            int p,q;
            for(p=0; p<n;p++){
                for(q=0; q<10;q++){
...
CODELETS(dgetrf_hcore, 1, cl_dgetrf_hcore_cpu_func, cl_dgetrf_hcore_cuda_func, 0)
#else
CODELETS_CPU(dgetrf_hcore, 1, cl_dgetrf_hcore_cpu_func)
#endif
//#endif
