...
//CHAMELEON_CL_CB(dgytlr,        starpu_matrix_get_nx(task->handles[0]), starpu_matrix_get_ny(task->handles[0]), 0,                                                M*N)
/*   MORSE_TASK_dgytlr - Generate a tile for random matrix. */
//#if defined(HICMA_DOUBLE)
void HICMA_TASK_dgytlr( const MORSE_option_t *options,
                        int m, int n,
                        ...
                        )
{
    ...
    //void (*callback)(void*) = options->profiling ? cl_dgytlr_callback : NULL;
