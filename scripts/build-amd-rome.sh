#!/bin/bash
repo=hicma-dev
hwloc_install_dir=$HOME/rabab/sourcefiles/hwloc-install
starpu_install_dir=$HOME/rabab/sourcefiles/starpu-1.2
...
if [ $run_module_setup -eq 1 ]; then
    . scripts/modules-amd-rome.sh
    pause_info
fi
cd $HOME/rabab/sourcefiles
...
if [ $compile_starpu -eq 1 ];then
    ...
    ./configure --prefix=$starpu_install_dir --disable-cuda --disable-opencl  --disable-build-doc --disable-export-dynamic --without-mpicc --enable-maxcpus=128 
    #--disable-mpi-check 
    make -j
    make -j install
...
if [ $compile_cham -eq 1 ];then
    ...
    mkdir build 
    cd build
    cmake .. -DCMAKE_INSTALL_PREFIX=$currentdir/$repo/chameleon/build/install -DCMAKE_COLOR_MAKEFILE:BOOL=ON -DCMAKE_VERBOSE_MAKEFILE:BOOL=ON -DBUILD_SHARED_LIBS=ON -DCHAMELEON_ENABLE_EXAMPLE=ON -DCHAMELEON_ENABLE_TESTING=OFF -DCHAMELEON_ENABLE_TIMING=ON -DCHAMELEON_USE_MPI=OFF -DCHAMELEON_USE_CUDA=OFF -DCHAMELEON_SCHED_QUARK=OFF -DCHAMELEON_SCHED_STARPU=ON
    make -j install
